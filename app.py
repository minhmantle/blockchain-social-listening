...
[data-testid="stSegmentedControl"] label{{background:#E8F5EE !important;border-color:#C8EAD8 !important;color:#4A7A5A !important;font-weight:600;}}
[data-testid="stSegmentedControl"] label[aria-checked="true"]{{background:#00A572 !important;border-color:#00A572 !important;color:#FFFFFF !important;}}
</style>
...
def main():
    ...
    # Lazy-load tabs: st.tabs() renders ALL tab content on every rerun (X API +
    # Claude calls for all 3 tabs), even for tabs the user isn't looking at.
    # st.segmented_control only runs the code for the currently selected view.
    TAB_MANTLE, TAB_COMPETITIVE, TAB_INTEL = (
        "📊  Mantle Deep Dive", "⚔️  Competitive Analysis", "🧠  Market Intelligence"
    )
    selected_view = st.segmented_control(
        "View",
        options=[TAB_MANTLE, TAB_COMPETITIVE, TAB_INTEL],
        default=TAB_MANTLE,
        label_visibility="collapsed",
        key="active_view",
    )
    if not selected_view:  # user can deselect a segmented_control -> fall back
        selected_view = TAB_MANTLE

    st.markdown("<div style='margin-top:8px'></div>", unsafe_allow_html=True)

    if selected_view == TAB_MANTLE:
        tab_mantle(token)
    elif selected_view == TAB_COMPETITIVE:
        tab_competitive(token)
    else:
        tab_intel(token)
